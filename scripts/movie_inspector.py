...
def main(argv=None):
    ...
    directors = extract_filtered_series(
        source_trimmed,
    ...
    actor_1 = extract_filtered_series(
        source_trimmed,
    ...
    actor_2 = extract_filtered_series(
        source_trimmed,
    ...
    actor_3 = extract_filtered_series(
        source_trimmed, 
    ...
    movie_keywords_out = os.path.join(
        'output', 'movies', 'movie_keywords-split.csv'
    )
    ...
    movie_country = extract_filtered_series(source_trimmed, ['movie_title', 'country'])
    movie_country_out = os.path.join('output', 'movies', 'movie_country.csv')
    write_series_to_csv(movie_country, movie_country_out, ',', False)
    logging.info(msg[11].format(os.path.abspath(movie_country_out)))
    ...
    logging.info(msg[12].format(os.path.abspath(country_out)))
    ...
    movie_language = extract_filtered_series(source_trimmed, ['movie_title', 'language'])
    ...
    movie_rating = extract_filtered_series(source_trimmed, ['movie_title', 'content_rating'])
...
def extract_filtered_series(data_frame, column_list):
    ...
    return data_frame[column_list].drop_duplicates().dropna(axis=0, how='all').sort_values(column_list)
